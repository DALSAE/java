...
public class PhoneBookManager_1 {//누구나 접근가능한 폰북매니져 클래스 선언?
	private static PhoneBookManager_1 pm;//
	...
	private PhoneBookManager_1() {}
	
	public static PhoneBookManager_1 getInstance() {
		if(pm==null) pm = new PhoneBookManager_1();
		return pm;
	}
	...
	void saveData() {
		try {
			...
			//이제까지 입력된 데이터를 싹 저장 하기? 이제까지 저장된 건 infostrage에 있는디
		}catch(IOException e){
			e.printStackTrace();
		}
		
		//파일을 만들어서 파일을 저장하는건가? 걍 데이터만 저장? 
	}
}
